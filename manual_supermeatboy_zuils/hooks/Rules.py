...
def progCharacter(world: World, multiworld: MultiWorld, state: CollectionState, player: int):
    ...
    if cfg.bandages:
        return (
            state.has("Josef", player) and state.has("Bandage", player, 30) or
...
def startingCharacter(world: World, multiworld: MultiWorld, state: CollectionState, player: int):
    cfg: GameConfig = initalize_globals(world, multiworld, player)
    return (
        state.has_group("ProgCharacters", player) or
        state.has("4bit/4color/8bit/Normal Meat Boy", player) or
        (cfg.starting_char == 8 and state.has("RunMan", player)) or
        (cfg.starting_char == 10 and state.has("Meat Ninja", player))
    ) 

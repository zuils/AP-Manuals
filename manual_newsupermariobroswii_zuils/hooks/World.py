...
def initialize_config(world: World, multiworld: MultiWorld, player: int) -> GameConfig:
    ...
    if goal == 1:
        star_coins_amount = 231
        world.options.star_coins_amount.value = 231
...
def after_create_regions(world: World, multiworld: MultiWorld, player: int):
    ...
    if cfg.goal != 1:
        for l in location_table:
            if "World 9" in l.get("category", []):
                locationNamesToRemove.append(l["name"])
...
def before_create_items_filler(item_pool: list, world: World, multiworld: MultiWorld, player: int) -> list:
    ...
    if not cfg.cannons:
        itemNamesToRemove.extend(f"World {i} Cannon Unlock" for i in range(1, 7))
    
    if cfg.goal != 1:
        itemNamesToRemove.extend(f"9-{i} Completion" for i in range(1, 9))
        loc = next(l for l in multiworld.get_unfilled_locations(player=player) if l.name == "8-Bowser's Castle Normal Exit")
    ...
    if cfg.goal == 2:
        for _ in range(7):
            loc = next(l for l in multiworld.get_unfilled_locations(player=player) if "Castle Normal Exit" in l.name)
            item = next(i for i in item_pool if i.name == "Boss Token")
            loc.place_locked_item(item)
            item_pool.remove(item)
    else:
        itemNamesToRemove.extend("Boss Token" for _ in range(7))
        
    if not cfg.tower_keys:
        itemNamesToRemove.extend(f"World {i} Tower Key" for i in range(1, 9))
        
    if cfg.world_unlocks == 0:
        itemNamesToRemove.extend(f"World {i} Castle Key" for i in range(1, 8))
        itemNamesToRemove.append("Bowser's Castle Key")
        if cfg.goal != 1:
            itemNamesToRemove.append("World 9 Key")
    else:
        itemNamesToRemove.extend(f"World {i} Key" for i in range(1, 10))
...
def after_set_rules(world: World, multiworld: MultiWorld, player: int):
    ...
    def goal(state: CollectionState) -> bool:
        if cfg.goal == 1:
            return state.has_group("World 9 Completions", player, 8)
        else:
